...
\title{\textbf{Tabulação, importação e exportação de dados}}
\subtitle{Recomendações gerais na tabulação, leitura e escrita de dados
usando R}
...
\begin{frame}[fragile]{Importação e exportação}
\protect\hypertarget{importauxe7uxe3o-e-exportauxe7uxe3o}{}
\begin{itemize}
\item
  Usando o \texttt{R} é possível \textbf{importar} e \textbf{exportar}
  dados em diferente formatos.
\item
  Os formatos comuns são \textbf{txt}, \textbf{csv} e \textbf{xls/xlsx}.
\item
  É possível trabalhar com outros formatos e até mesmo bancos de dados
  relacionais.
\item
  Mas para que a leitura seja bem sucedida, uma etapa anterior merece
  atenção: a \textbf{tabulação dos dados}.
\end{itemize}
\end{frame}
...
\begin{frame}{Tabulação}
\protect\hypertarget{tabulauxe7uxe3o-1}{}
\begin{itemize}
\item
  Em Estatística e análise de dados de modo geral trabalhamos com
  \textbf{dados estruturados} em um \textbf{formato tabular}.
  ...
  \item
    Cada \textbf{célula} o valor da variável para aquela
    \textbf{observação}.
  \end{itemize}
\item
  A ideia é simples, mas na prática\ldots{} Vemos muitas
  inconsistências.
\item
  Uma tabulação adequada economiza tratamentos posteriores para realizar
  a leitura dos dados e, consequentemente, adianta a análise.
\end{itemize}
\end{frame}

\begin{frame}{Inconsistências comuns}
\protect\hypertarget{inconsistuxeancias-comuns}{}
\begin{enumerate}
\item
  Níveis de variáveis qualitativas tabulados de forma inconsistente.
\item
  Variáveis quantitativas representadas com caracteres.
\item
  Inconsistência nas escalas.
\item
  Inconsistência nos decimais.
\item
  Valores ausentes tabulados de forma equivocada.
\item
  Planilhas formatadas (células mescladas, subtabelas coloridas).
\item
  Variáveis de interesse em múltiplas abas de uma planilha.
\item
  Codificações que só fazem sentido para o pesquisador.
\item
  A mesma variável tabulada em diferentes colunas (comum em estudos
  longitudinais).
\end{enumerate}
\end{frame}

\begin{frame}{Dicas e boas práticas}
\protect\hypertarget{dicas-e-boas-pruxe1ticas}{}
\begin{itemize}
\item
  Na tabulação evite nomes de colunas longos ou caracteres especiais.
\item
  Seja consistente na tabulação dos níveis de variáveis qualitativas
  (maiúsculas e mínusculas, espaços, abreviações).
\item
  Se a variável é quantitativa, não coloque texto nas células.
\item
  Defina qual será o representador de decimal e use-o em toda a
  tabulação.
\item
  Se o valor for ausente, coloque NA na célula ou deixe-a em branco.
\item
  Não use células mescladas se seu interessee é importar os dados.
\item
  Gere uma tabela ``final'' com as variáveis que você tem interesse em
  analisar. Evite várias planilhas ou abas com diferentes variáveis que
  devem ser analisadas juntas.
\item
  Evite códigos e procedimentos que só fazem sentido pra você. Você está
  habituado com os dados, quem vai analisar não.
\end{itemize}
\end{frame}
...
\protect\hypertarget{importauxe7uxe3o-1}{}
\begin{itemize}
\item
  Chamamos de \textbf{importação} ou \textbf{leitura} o procedimento de
  entrar com um conjunto de dados externo no R.
\item
  ...
  \item
    Arquivos de dados do \texttt{R} como RData, rda, rds.
  \item
...
\begin{frame}[fragile]{Importação}
\protect\hypertarget{importauxe7uxe3o-2}{}
\begin{itemize}
\item
  Vamos explorar os comandos para leitura dos tipos mais comuns:
  arquivos txt, csv e xlsx/xls.

  \begin{itemize}
  \item
    Para arquivos txt: \texttt{read.table()}.
  \item
    Para arquivos csv: \texttt{read.csv()} e \texttt{read.csv2()}.
  \item
    Para planilhas do Excel (arquivos xls e xlsx) precisamos do auxílio
    de bibliotecas para fazer a importação, como a \texttt{openxlsx} que
    possui a função \texttt{read.xlsx()}.
  \end{itemize}
\item
  Existem formas alternativas para entrar com conjuntos de dados de
  forma manual no R, como as funções \texttt{scan()} e \texttt{edit()}.
\end{itemize}
\end{frame}

\hypertarget{exportauxe7uxe3o}{%
\section{Exportação}\label{exportauxe7uxe3o}}

\begin{frame}[fragile]{Exportação}
\protect\hypertarget{exportauxe7uxe3o-1}{}
\begin{itemize}
\item
  Chamamos de \textbf{exportação} o procedimento de enviar um conjunto
  de dados para fora do R como um arquivo.
\item
  Considerando os já mencionados tipos (txt, csv e xlsx), podemos usar
  as funções:

  \begin{itemize}
  \item
    \texttt{write.table()} para exportar arquivos txt e csv.
  \item
    \texttt{write.csv()} para arquivos csv.
  \item
    \texttt{write.xlsx()} do pacote \texttt{openxlsx}.
  \end{itemize}
\end{itemize}
\end{frame}

\begin{frame}[fragile]{O que foi visto}
\protect\hypertarget{o-que-foi-visto}{}
...
\tightlist
\item
  Entrada de dados no R com as funções \texttt{read.table()},
  \texttt{read.csv()}, \texttt{read.xlsx()}, \texttt{scan()} e
  \texttt{edit(data.frame())}.
\end{itemize}
...
\tightlist
\item
  Saída de dados do R com as funções \texttt{write.table()},
  \texttt{write.csv()} e \texttt{write.xlsx()}
\end{itemize}
\end{frame}
